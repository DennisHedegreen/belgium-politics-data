...
class PublicSurfaceContractTests(unittest.TestCase):
    def test_single_country_registry(self):
        countries = list_exposed_countries(["belgium"], allow_internal=False)
        self.assertEqual(len(countries), 1)
        self.assertEqual(countries[0].country_id, "belgium")
    ...
    def test_country_config_matches_public_surface(self):
        config = get_country_config("belgium")
        self.assertEqual(config.display_name, "Belgium")
        self.assertEqual(config.adjective, "Belgian")
        self.assertEqual(len(config.supported_factors), 3)
    ...
    def test_public_wrapper_defaults(self):
        ...
        self.assertIn('WPD_APP_TITLE", "Belgian Politics Data"', wrapper)
        self.assertIn('WPD_EXPOSE_COUNTRIES", "belgium"', wrapper)
        self.assertIn('WPD_PROFILE", "belgium_only"', wrapper)
